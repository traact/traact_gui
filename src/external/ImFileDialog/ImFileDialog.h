...
namespace ifd {
	...
	public:
		...
		inline const fs::path& GetResult() { return m_result[0]; }
		inline const std::vector<fs::path>& GetResults() { return m_result; }
	...
	private:
		...
		std::vector<fs::path> m_selections;
		int m_selectedFileItem;
		void m_select(const fs::path& path, bool isCtrlDown = false);

		std::vector<fs::path> m_result;
		...
		void* m_getIcon(const fs::path& path);
		void m_clearIcons();
		...
		void m_setDirectory(const fs::path& p, bool addHistory = true);
