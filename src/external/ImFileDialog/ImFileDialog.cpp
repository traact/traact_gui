...
namespace ifd {
	...
	bool PathBox(const char* label, fs::path& path, char* pathBuffer, ImVec2 size_arg) {
	...
	FileDialog::FileData::FileData(const fs::path& path) {
		std::error_code ec;
		Path = path;
	...
	FileDialog::FileDialog() {
		...
		m_setDirectory(fs::current_path(), false);
		...
		thisPC->Read = true;
		if (fs::exists(userPath + L"3D Objects"))
	...
	bool FileDialog::Save(const std::string& key, const std::string& title, const std::string& filter, const std::string& startingDir)
	{
		...
		if (!startingDir.empty())
			m_setDirectory(fs::u8path(startingDir), false);
		else
	...
	{
		m_currentKey.clear();
		m_backHistory = std::stack<fs::path>();
		m_forwardHistory = std::stack<fs::path>();
	...
	void FileDialog::m_select(const fs::path& path, bool isCtrlDown)
	{
	...
	void* FileDialog::m_getIcon(const fs::path& path)
	{
#ifdef _WIN32
	...
	void FileDialog::m_setDirectory(const fs::path& p, bool addHistory)
	{
		...
		if (p.u8string().size() == 2 && p.u8string()[1] == ':')
			m_currentDirectory = fs::u8path(p.u8string() + "\\");
#endif
		...
		if (p.u8string() == "Quick Access") {
			...
			std::error_code ec;
			if (fs::exists(m_currentDirectory, ec))
			...
			if (ImGui::BeginTable("##contentTable", 3, /*ImGuiTableFlags_Resizable |*/ ImGuiTableFlags_Sortable, ImVec2(0, -FLT_MIN))) {
				...
				for (auto& entry : m_content) {
					...
					if (ImGui::Selectable(filename.c_str(), isSelected, ImGuiSelectableFlags_SpanAllColumns | ImGuiSelectableFlags_AllowDoubleClick)) {
						std::error_code ec;
						bool isDir = fs::is_directory(entry.Path, ec);
	...
	void FileDialog::m_renderPopups()
	{
		...
		if (ImGui::BeginPopupModal("Enter directory name##newdir")) {
			...
			if (ImGui::Button("OK")) {
				std::error_code ec;
				fs::create_directory(m_currentDirectory / std::string(m_newEntryBuffer), ec);
				m_setDirectory(m_currentDirectory, false); // refresh
				m_newEntryBuffer[0] = 0;
				ImGui::CloseCurrentPopup();
			}
	...
	void FileDialog::m_renderFileDialog()
	{
		...
		if (ImGui::ArrowButtonEx("##back", ImGuiDir_Left, ImVec2(GUI_ELEMENT_SIZE, GUI_ELEMENT_SIZE), m_backHistory.empty() * ImGuiItemFlags_Disabled)) {
			fs::path newPath = m_backHistory.top();
			m_backHistory.pop();
		...
		if (ImGui::ArrowButtonEx("##forward", ImGuiDir_Right, ImVec2(GUI_ELEMENT_SIZE, GUI_ELEMENT_SIZE), m_forwardHistory.empty() * ImGuiItemFlags_Disabled)) {
			fs::path newPath = m_forwardHistory.top();
		...
		fs::path curDirCopy = m_currentDirectory;
