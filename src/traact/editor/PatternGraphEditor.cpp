    ...
    for (const auto &pattern : Patterns) {
        ...
        for (const auto &input_pin : node->Inputs) {
            ...
            if (input_pin->TraactPort->isConnected()) {
                ...
                if (!output_pin) {
                    spdlog::error("node editor pin not found for {0}:{1}",
                                  input_pin->TraactPort->connected_to.first,
                                  input_pin->TraactPort->connected_to.second);
                    continue;
                }
...
DFGNode::Ptr PatternGraphEditor::FindNode(ax::NodeEditor::NodeId id) {
    ...
    auto result =
        std::find_if(Patterns.begin(), Patterns.end(), [id](auto &pattern) { return pattern->DfgNode->ID == id; });
...
DFGLink::Ptr PatternGraphEditor::FindLink(ax::NodeEditor::LinkId id) {
    auto result =
        std::find_if(DfgConnections.begin(), DfgConnections.end(), [id](auto &link) { return link->ID == id; });
...
bool PatternGraphEditor::IsPinLinked(ax::NodeEditor::PinId id) {
    ...
    auto result = std::find_if(DfgConnections.begin(),
                               DfgConnections.end(),
                               [id](auto &link) { return link->StartPinID == id || link->EndPinID == id; });
    if (result != DfgConnections.end())
        return true;
...
void PatternGraphEditor::DisconnectPin(ax::NodeEditor::PinId endPin) {
    auto result = std::find_if(DfgConnections.begin(),
                               DfgConnections.end(),
                               [endPin](auto &link) { return link->EndPinID == endPin; });
    if (result != DfgConnections.end()) {
...
PatternGraphEditor::CanMergeNodes(ax::NodeEditor::NodeId node1, ax::NodeEditor::NodeId node2) {
    ...
    auto add_all =
        [](std::vector<const pattern::spatial::CoordinateSystem *> &container, const SRGMergedNode::Ptr &mergedNode) {
    ...
    auto contains_any_cs =
        [this](const pattern::Pattern &pat, std::vector<const pattern::spatial::CoordinateSystem *> &cs) {
            for (const auto &local_cs : cs) {
                for (const auto &port_group : pat.port_groups) {
                    for (const auto &name_cs : port_group.coordinate_systems) {
...
void PatternGraphEditor::SplitNode(ax::NodeEditor::NodeId id) {
    ...
    auto add_or_create_node_group = [&new_node_groups](const SRGNode::Ptr &node) {
        ...
        for (auto &edge : node->Edges) {
            ...
            auto result_groups =
                std::find_if(new_node_groups.begin(), new_node_groups.end(), [&edge, &node](const auto &node_group) {
    ...
    for (std::size_t i = 0; i < new_node_groups.size(); ++i) {
        if (new_node_groups[i].size() > 1) {
            ...
            newMergeNode->Position = merge_node->Position
                + ImVec2(std::sin(split_factor * i) * split_distance, std::cos(split_factor * i) * split_distance);
            ...
            local_node->Position = merge_node->Position
                + ImVec2(std::sin(split_factor * i) * split_distance, std::cos(split_factor * i) * split_distance);
