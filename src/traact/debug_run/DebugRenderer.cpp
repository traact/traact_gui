...
DebugRenderer::DebugRenderer() : scene_window_(std::make_shared<DebugSceneWindow>("DebugScene", this)) {
    ...
    profile_window_ = std::make_shared<DataflowProfileWindow>("Profile", this);
    render_components_["Profile"].emplace_back(profile_window_);
}
...
void DebugRenderer::setDataflowState(dataflow::DataflowState::SharedPtr dataflow_state) {
    profile_window_->init(dataflow_state);
}
} // traact